...
class DatabaseOpenHelper extends SQLiteOpenHelper {
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		try {
			for (int currentVersion = oldVersion; currentVersion < newVersion; currentVersion++) {
				// 差异更新
				ArrayList<String> scripts = getUpgradeScript(currentVersion, currentVersion + 1);
				EvtLog.d(TAG, "update database version from :" + currentVersion + " to: " + (currentVersion + 1));
				// 无升级脚本，则默认先删除表，再创建表
	...
	private void ormCreate(SQLiteDatabase db, String ormCreateString) {
		...
		try {
			...
			String sqlStr = mBuilder.getSQLCreate(Utils.toSQLName(table));
			if (sqlStr != null) {
				db.execSQL(sqlStr);
			}
		} catch (Exception e) {
	...
	private void ormDropTable(SQLiteDatabase db, String ormDropString) {
		...
		try {
			...
			String table = ormDropString.substring(ormDropString.indexOf("(") + 1, ormDropString.indexOf(")"));
			if (!StringUtil.isNullOrEmpty(table)) {
				sqlStr = mBuilder.getSQLDrop(Utils.toSQLName(table));
				if (sqlStr != null) {
					db.execSQL(sqlStr);
				}
			} else {
				cursor = db.query(true, "sqlite_master", new String[] { "name" }, "type=\"table\"", null, null, null,
						null, null);
				// 循环删除所有表
				while (cursor.moveToNext()) {
					table = cursor.getString(0);
					if ("".equals(table.trim()) || table.trim().startsWith(sysTablePrefix)) {
						continue;
					}
	...
	private ArrayList<String> getUpgradeScript(int oldVersion, int newVersion) {
		...
		try {
			...
			InputStream inputStream = assetMgr.open(mBuilder.getDatabaseName() + "_" + oldVersion + "_" + newVersion);
